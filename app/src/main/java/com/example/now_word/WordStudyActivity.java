...
public class WordStudyActivity extends AppCompatActivity implements RadioGroup.OnCheckedChangeListener, View.OnClickListener {
    ...
    Map<Word,Integer> five_five_map;//记录今天单词的五五循环次数
    Iterator<Word> iterator_five;
    Iterator<Word> iterator;//新背和复习的列表迭代器
    Word nowword;   //现在正在背的单词
    boolean five_flag = false;//五五循环的标志
    int today_needreviewcount;//记录还需复习的单词数量
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_word_study);    //绑定布局文件
        init();//初始化控件
    }
    ...
    public void init() {
        ...
        needCount=settingDao.getNewNum();//让复习和听写的单词数与设置中新背单词数一致
        ...
        }else if(studyWords.equals("need_review")){
            needstudy= wordRecordDao.getNeedReWords(wordRecordDao.getTypeReWordCount());
        }
        iterator=needstudy.iterator();
        today_needreviewcount=needstudy.size();//学习单词的总数量
        size0 = needstudy.size();//学习单词的总数量

        if (size0<=5){Toast.makeText(this, "你此类单词太少，快去背单词吧！", Toast.LENGTH_LONG).show();
            finish();}
        else {
            getNextData();
        }
    ...
    private void getNextData() {
        ...
        if (iterator_five.hasNext()) {  //五五循环未结束,复习今日的五五循环
            ...
            if (iterator.hasNext()) {//如果没有学完要学的词
                ...
                //更新左上角还要背的单词数量信息
                needfinishCountText.setText(String.valueOf(today_needreviewcount));
